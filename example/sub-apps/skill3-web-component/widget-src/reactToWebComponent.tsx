 ...
 *  - 소비하는 쪽(Mother App)은 React를 몰라도 된다 — 그냥 <script>로 불러
 *    <tag-name> 을 쓰면 된다. React 런타임은 빌드 시 위젯 번들 안에 포함되어
 *    Mother App의 React와 공유되지 않는다(버전 충돌/전역 오염 없음).
 *  - Shadow DOM에 마운트해 스타일/DOM을 격리한다(전역 CSS 오염 방지).
 *
 * 매핑:
 *  - observedAttributes: HTML attribute → React prop (문자열)
 *  - properties: JS property(예: portalAuth 객체) → React prop
 *    (attribute로 못 넘기는 객체/함수를 property로 주입할 때)
 */
export function defineReactWebComponent(
  ...
  // 객체/함수 주입용 property (예: portalAuth). connect 전에 세팅돼도 props에 보관됐다가
  // connect 시점에 렌더된다.
